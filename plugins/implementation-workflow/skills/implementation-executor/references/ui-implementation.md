...
- UI copy は、ユーザーへ行動を指示する言い方ではなく、判断材料を中立に提示する言い方にする。判断基準と Before / After は `../../implementation-planner/references/ui-ux.md` の Result-Centered Copy を正本とし、定型置換はしない。
...
- ユーザー向け文言が行動指示型になっていないかを、`ui-ux.md` の Result-Centered Copy に照らして確認する。
