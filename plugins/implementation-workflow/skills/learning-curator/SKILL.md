...
| Mode | Trigger | 扱う量 | 書き込み前の承認 |
| --- | --- | --- | --- |
| Incident | 直前の訂正 1 件、hook が検知した correction | 1 件 | 明示的 durable intent があれば不要 |
| Retrospective | `retro`、または user が session の振り返りを依頼 | 複数候補 | 候補を提示し、user の選択が必要 |

promotion gate はどちらの mode でも同じ。Retrospective mode の手順と evidence 境界は `references/retrospective-sweep.md` を正本にする。
...
- Retrospective mode では、context に残る会話、git state、accepted review から複数の候補を洗い出し、human の選択を得てから昇格する。
...
- Retrospective mode での user の起動は batch 単位の durable intent であり、個々の候補への承認ではない。書き込み前に候補を提示し、選択を得る。
...
1. mode を決める。直前の訂正 1 件なら Incident、session の振り返り依頼なら Retrospective とし、後者は `references/retrospective-sweep.md` の手順で候補を洗い出し、選択を得てから 5 以降へ進む。
2. correction の source を特定する。human の直接訂正、accepted review comment、human-authored fix、deterministic failure のどれかを明示する。
3. current task の誤りを修正し、期待する behavior を evidence で確認する。
4. `references/learning-lifecycle.md` で one-off clarification、changed requirement、reusable mistake class を分類し、promotion gate を判定する。
5. target scope を決める。current file / subtree / repository / organization / personal の順に、必要以上に広げない。
6. nearest `AGENTS.md` / `CLAUDE.md`、既存 skill / references、rules、hooks、tests、linters を検索する。symlink、import、重複、矛盾を確認する。
7. `references/destination-routing.md` で canonical destination と enforcement level を選ぶ。
8. candidate を atomic にする: `Trigger/Scope → Constraint → Rationale → Verification`。conversation-specific noun、temporary path、issue numberだけの rule は避ける。
9. `ADD` / `REFINE` / `MERGE` / `NOOP` を決め、localized delta だけを適用する。新しい artifact は既存 artifact で表現できない時だけ作る。
10. `references/evaluation.md` に従い、structure check、trigger fixture、false-positive fixture、behavior replay のうち変更リスクに必要なものを実行する。
11. 変更先、operation、根拠、verification、残る限界を簡潔に報告し、`Learning outcome: ...` で終了する。
